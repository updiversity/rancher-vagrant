...
while true; do
  ENV_ID=$(docker run \
    ...
    $curlprefix/curl \
      $rancher_api_auth \
      -sLk \
      "$protocol://$rancher_server_ip/v2-beta/project?name=$env_name" | jq '.data[0].id' | tr -d '"')
...
docker run \
  ...
  $curlprefix/curl \
    ...
    "$protocol://$rancher_server_ip/v2-beta/projects/$ENV_ID/registrationtokens/?state=active" |
      jq -r .data[].command |
      head -n1 |
      sed "s/sudo\ docker\ run/sudo\ docker\ run\ -e\ CATTLE_AGENT_IP=$node_ip/" |
      sh
