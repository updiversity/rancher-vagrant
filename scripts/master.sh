...
echo "        zone \"rancher.$dnstld\" {
             type master;
             file \"/etc/bind/db.rancher.$dnstld\";
...
;
\$TTL    604800
@       IN      SOA     rancher.$dnstld. admin.rancher.$dnstld. (
                              1         ; Serial
                         604800         ; Refresh
                          86400         ; Retry
                        2419200         ; Expire
...
;
@       IN      NS      rancher.$dnstld.
rancher.$dnstld.     IN      A       $cache_ip" > /root/db.rancher.$dnstld

docker run -d --restart=always --name bind9 -p 53:53 -p 53:53/udp -v /root/named.conf.local:/etc/bind/named.conf.local -v /root/bind.conf:/etc/bind/named.conf -v /root/db.rancher.$dnstld:/etc/bind/db.rancher.$dnstld resystit/bind9:latest
